...
def DeepIDA_nonBootstrap(data_train, data_valid, data_test, Y_train, Y_valid, Y_test, 
                         structure, TS_num_features, lr_rate = 0.01, n_epoch = 50 ):
...
def DeepIDA_VS(data_all,Y,n_boot,structure,n_epoch,variables_name_list):
    ...
    for i in range(n_boot):
        all_to_run.append((data_all,Y,i,i,structure,n_epoch))   
    ...
    for j in range(len(data_all)):
        ...
        for i in range(n_boot):
            one_view_eff_list.append(outputs[i][1][j])
            one_view_select_list.append(outputs[i][2][j])
    ...
    for i in range(len(data_all)):
        ...
        df.sort_values(by=['eff_prop'],ascending=False).to_csv("view_"+str(i+1)+".csv",index=False)
...
def DeepIDA_Bootstrap(data_train, data_valid, data_test, Y_train, Y_valid, Y_test, 
                         structure,n_boot, n_epoch_boot,variables_name_list, TS_num_features, lr_rate = 0.01, n_epoch_nonboot = 50 ):
    ...
    VS_result = DeepIDA_VS(data_VS,Y_VS,n_boot,structure,n_epoch_boot,variables_name_list)
    ...
    final = DeepIDA_nonBootstrap(data_train_s, data_valid_s, data_test_s, Y_train, Y_valid, Y_test, structure, TS_num_features, lr_rate, n_epoch_nonboot)
